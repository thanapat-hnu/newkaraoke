...
function doRegister() {
  ...
  if (!name) { showErr('Name', 'กรุณากรอกชื่อ'); err = true; }
  if (!phone) { showErr('Phone', 'กรุณากรอกเบอร์โทร'); err = true; }
  else if (!phoneRegex.test(phone)) { showErr('Phone', 'เบอร์โทรต้องเป็นตัวเลข 10 หลัก'); err = true; }
  if (!email) { showErr('Email', 'กรุณากรอกอีเมล'); err = true; }
  else if (!emailRegex.test(email)) { showErr('Email', 'รูปแบบอีเมลไม่ถูกต้อง'); err = true; }
  if (!pass) { showErr('Password', 'กรุณากรอกรหัสผ่าน'); err = true; }
  else if (pass.length < 6) { showErr('Password', 'รหัสผ่านอย่างน้อย 6 ตัว'); err = true; }
...
function clearErrors() {
  ['Name', 'Phone', 'Email', 'Password'].forEach(f => {
